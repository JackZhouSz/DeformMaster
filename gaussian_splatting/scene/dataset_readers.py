...
def readQQTTSceneInfo(
    ...
    # Read resolution from first image instead of hardcoding (was 848x480)
    _first_img_path = os.path.join(path, "0.png")
    if os.path.exists(_first_img_path):
        _first_img = Image.open(_first_img_path)
        W, H = _first_img.size
    else:
    ...
    test_cam_infos = []
    interp_poses_path = os.path.join(path, "interp_poses.pkl")
    test_c2ws = (
        pickle.load(open(interp_poses_path, "rb"))
        if os.path.exists(interp_poses_path)
        else []
    )
    for cam_i, c2w in enumerate(test_c2ws):
        dummy_cam_id = 1
    ...
    # getNerfppNorm uses camera-center spread for scene radius, which collapses
    # to 0 for single-camera (mono) setups. That makes densify_and_prune treat
    # every gaussian as "too big" (scale > 0.1 * extent == 0) and wipe them all
    # out. Fall back to the point-cloud bounding sphere when the camera baseline
    # is degenerate.
    if nerf_normalization["radius"] < 1e-3:
        pcd_center = all_xyz.mean(axis=0)
        pcd_radius = float(np.linalg.norm(all_xyz - pcd_center, axis=1).max())
        nerf_normalization = {
            "translate": -pcd_center,
            "radius": pcd_radius * 1.1,
        }
        print(
            f"  [mono] nerf_normalization fallback: radius={nerf_normalization['radius']:.3f}"
        )
