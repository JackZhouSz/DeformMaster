...
class MPM_Simulator_WARP:
    ...
    def p2g2p(self, step, dt, device="cuda:0", moe_params=None, svd_clamp_max=2.0):
        grid_size = (
        ...
        with wp.ScopedTimer(
            ...
            if moe_params is not None:
                # Use Mixture of Experts Stress
                from .moe_utils import compute_moe_stress_kernel
                wp.launch(
                    ...
                    inputs=[
                        ...
                        moe_params['fk'],
                        moe_params['fdir'],
                        self.mpm_state.particle_stress,
                        moe_params['active_mask'],
                        svd_clamp_max
                    ],
                    device=device
                )
                # Update F (assume elastic for now or handle return mapping separately)
                # For MoE, we often assume F = F_trial unless plasticity is added
                # self.mpm_state.particle_F = self.mpm_state.particle_F_trial
                wp.launch(kernel=set_value_mat33, dim=self.n_particles, 
                          inputs=[self.mpm_state.particle_F, self.mpm_state.particle_F_trial], device=device)
            else:
                wp.launch(
    ...
    def reset_densities_and_update_masses(
        ...
        self.mpm_state.particle_density = torch2warp_float(
            all_particle_densities, dvc=device, requires_grad=True
        )
        wp.launch(
            kernel=get_float_array_product,
            dim=self.n_particles,
    ...
    def import_particle_x_from_torch(self, tensor_x, clone=False, device="cuda:0"):
        if tensor_x is not None:
            if clone:
                tensor_x = tensor_x.clone()
            self.mpm_state.particle_x = torch2warp_vec3(tensor_x, dvc=device, requires_grad=True)

    def import_particle_v_from_torch(self, tensor_v, clone=False, device="cuda:0"):
        if tensor_v is not None:
            if clone:
                tensor_v = tensor_v.clone()
            self.mpm_state.particle_v = torch2warp_vec3(tensor_v, dvc=device, requires_grad=True)
    ...
    def import_particle_F_from_torch(self, tensor_F, clone=True, device="cuda:0"):
        if tensor_F is not None:
            ...
            self.mpm_state.particle_F = torch2warp_mat33(tensor_F, dvc=device, requires_grad=True)

    def import_particle_C_from_torch(self, tensor_C, clone=True, device="cuda:0"):
        if tensor_C is not None:
            if clone:
                tensor_C = tensor_C.clone().detach()
            tensor_C = torch.reshape(tensor_C, (-1, 3, 3))
            self.mpm_state.particle_C = torch2warp_mat33(tensor_C, dvc=device, requires_grad=True)
