...
def filter_track(track_path, pcd_path, mask_path, frame_num, cams):
    """``cams`` is a list of cam ids; processed_masks/points/colors arrays are
    indexed by *position* in cams, but the per-cam cotracker .npz files are
    keyed by cam_id."""
    ...
    for pos, cam_id in enumerate(cams):
        ...
        # Clip to valid image bounds (CoTracker can return boundary coords
        # that round to H or W, e.g., 359.5 → 360 for H=360)
        H, W = np.load(f"{pcd_path}/0.npz")["points"].shape[1:3]
        ...
        object_mask = processed_masks[0][pos]["object"]
        ...
        controller_mask = processed_masks[0][pos]["controller"]
        ...
        for frame_idx in range(1, frame_num):
            ...
            object_mask = processed_masks[frame_idx][pos]["object"]
            for j in range(num_points):
                try:
            ...
            controller_mask = processed_masks[frame_idx][pos]["controller"]
            for j in range(num_points):
        ...
        for frame_idx in range(frame_num):
            ...
            track_points[frame_idx][np.where(visibility[frame_idx])] = points[pos][
                tracks[frame_idx, np.where(visibility[frame_idx])[0], 0],
                tracks[frame_idx, np.where(visibility[frame_idx])[0], 1],
            ]
            track_colors[frame_idx][np.where(visibility[frame_idx])] = colors[pos][
                tracks[frame_idx, np.where(visibility[frame_idx])[0], 0],
                tracks[frame_idx, np.where(visibility[frame_idx])[0], 1],
            ]
