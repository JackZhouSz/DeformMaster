...
VIS = True  # final_matching.mp4 is useful for quality check
...
def registration_pnp(mesh_matching_points, raw_matching_points, intrinsic):
    ...
    n_corr = len(mesh_matching_points)
    if n_corr < 3:
        raise RuntimeError(
            f"Only {n_corr} valid mesh<->image correspondence(s) for case "
            f"'{case_name}'; PnP needs >=3. The object is likely too thin or "
            f"textureless for SuperGlue-based shape-prior alignment. Consider "
            f"setting shape_prior=False or using a preset mesh for this case."
        )
    # SQPNP is globally optimal and (unlike EPNP) supports as few as 3
    # correspondences -- important for thin/textureless objects (e.g. twine)
    # where SuperGlue finds very few matches.
...
if __name__ == "__main__":
    ...
    # Clean mesh topology for ARAP stability (TRELLIS 2 meshes are often
    # non-manifold with degenerate faces, causing Laplacian factorization failure)
    ...
    # ARAP can fail on high-aspect-ratio meshes (e.g., rope) because the
    # Cotangent Laplacian becomes ill-conditioned. Fall back to the rigid
    # PnP+Rescale alignment which is already a reasonable result.
    try:
        ...
        print(f"[align] ARAP failed ({e}), using rigid alignment as fallback")
        final_mesh_world = initial_mesh_world

    # Export final_mesh.glb BEFORE viz (so even if viz crashes, mesh is saved)
    ...
    if VIS:
        try:
            ...
            print(f"[align] viz failed ({e}), final_mesh.glb already saved")
