...
def render_image(mesh, camera_poses, width=640, height=480, fov=1, device="cpu"):
    ...
    # Fallback for textureless GLBs: SoftPhongShader.sample_textures will
    # raise "Meshes does not have textures" without this. Some exporters
    # (meshy.ai, raw trimesh exports) embed materials in a way pytorch3d's
    # MeshGlbFormat doesn't pick up — we just attach a uniform white
    # TexturesVertex. Alignment uses SuperGlue features on the rendered
    # silhouette + depth, which still works fine on a flat-colored mesh.
    if mesh.textures is None:
        verts = mesh.verts_padded()                       # (1, V, 3)
        mesh.textures = TexturesVertex(
            verts_features=torch.ones_like(verts)
        )
