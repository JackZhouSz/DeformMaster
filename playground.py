...
DEFAULT_CONFIG_PATH = "configs/volumetric.yaml"
KNOWN_CONFIG_PATHS = [
    "configs/volumetric.yaml",
    "configs/planar.yaml",
    "configs/linear.yaml",
]
...
class GradioInteractiveMPMPlayground:
    def __init__(
        ...
        n_ctrl_parts=1,
        inv_ctrl=False,
        output_dir="datasets/physics_flow_mpm",
        settle_iters=220,
        control_mode="mouse",
        rec_max_frames=0,
        rec_no_trail=True,
    ...
    with gr.Blocks(title="DeformMaster Interactive Playground (MPM)") as demo:
        ...
        with gr.Row():
            with gr.Column(scale=3):
                ...
                with gr.Row():
                    ctrl_stiff_slider = gr.Slider(0.1, 10.0, value=4.5, step=0.05, label="Controller Stiffness Scale")
...
def resolve_from_output(output, case_name):
    ...
    for cat in CATEGORY_DIRS:
        scene_dir = os.path.join(output, cat, case_name)
    ...
    raise FileNotFoundError(
        f"Cannot find config.yaml + best_checkpoint.pt for '{case_name}' in {output}/*/. "
        f"Searched: {', '.join(CATEGORY_DIRS)}"
    )
...
def main():
    ...
    parser.add_argument("--output", type=str, required=True,
                        help="Checkpoint dir for the scene (e.g. ./checkpoints). Auto-resolves config and checkpoint.")
    parser.add_argument("--base_path", type=str, default="./data")
    parser.add_argument("--gaussian_path", type=str, default="./checkpoints/gaussian")
    ...
    parser.add_argument("--output_dir", type=str, default="datasets/physics_flow_mpm")
    ...
    if overlay is None:
        raise FileNotFoundError(
            ...
            f"(try --bg_img_path <path> or --bg_mono)"
        )
