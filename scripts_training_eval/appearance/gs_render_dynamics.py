...
import copy
from scripts_training_eval.appearance.gs_render import (
...
def render_set(
    ...
    view_indices = [i for i in [0, 50, 100] if i < len(views)]
    if not view_indices:
        view_indices = [0]
...
def render_sets(
    ...
    with torch.no_grad():
        ...
        if not views:
            # mono / <3-cam scenes skip interp_poses, so test cameras are empty.
            # Fall back to train cameras to still produce renders.
            print("[mono] No test cameras, falling back to train cameras.")
