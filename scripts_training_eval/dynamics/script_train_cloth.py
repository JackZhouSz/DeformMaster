"""
Cloth scene batch trainer — multi-GPU parallel training for cloth/package cases.

Usage:
  # Use a config file. Required runtime settings are read from YAML.
  python scripts_training_eval/dynamics/script_train_cloth.py --config configs/cloth.yaml
"""
...
def worker(gpu_id, task_queue, config_path, iters):
    ...
    while True:
        ...
        print(f"\n[GPU {gpu_id}] >>> [CLOTH MODE] Training Scene: {scene_id} using {config_path}", flush=True)
...
def main():
    ...
    if 'target_scenes' in cfg and cfg.target_scenes:
        ...
        if len(valid_scenes) != len(targets):
            print(f"WARNING: Some target scenes were not found in {base_path}")

    print(f"Found {len(valid_scenes)} CLOTH scenes. Using GPU(s): {gpus}. Config: {args.config}", flush=True)
